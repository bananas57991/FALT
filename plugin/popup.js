document.getElementById("decode").addEventListener("click", async () => {
    ...
    try {
        ...
        const { ip } = parseIpPort(result);
        const host = input.slice(input.indexOf(".") + 1);

        await fetch(`http://127.0.0.1:8888/?ip=${encodeURIComponent(ip)}&host=${encodeURIComponent(host)}`);

        browser.tabs.create({
            url: "http://127.0.0.1:8000/ghost_latest.html"
        });
...
function parseIpPort(result) {
    if (result.startsWith("[")) {
        const match = result.match(/^\[(.+)\]:(\d+)$/);
        if (!match) throw new Error("Invalid IPv6 format");

        return { ip: match[1], port: match[2] };
    }

    const parts = result.split(":");
    if (parts.length !== 2) throw new Error("Invalid IPv4 format");
